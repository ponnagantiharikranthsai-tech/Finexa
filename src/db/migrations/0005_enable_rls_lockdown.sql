-- ==============================================================================
-- FINEXA EMERGENCY SECURITY LOCKDOWN MIGRATION
-- MIGRATION ID: 0005_enable_rls_lockdown.sql
-- DESCRIPTION: Enable RLS across all 15 public tables, revoke public/anon access,
--              enforce strict admin-only authorization, protect borrower data.
-- ==============================================================================

-- ------------------------------------------------------------------------------
-- 1. REVOKE ALL PRIVILEGES ON BUSINESS TABLES FROM 'anon' AND 'authenticated'
-- ------------------------------------------------------------------------------
...
-- Ensure service_role has complete access for backend & administrative operations
...
-- ------------------------------------------------------------------------------
-- 3. CLEAN UP ANY EXISTING RLS POLICIES ON PUBLIC TABLES
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 4. AUTHORIZATION FUNCTION: CHECK FOR AUTHENTICATED FINEXA OWNER/ADMIN
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 5. GRANT STRICT LEAST-PRIVILEGE ACCESS TO AUTHENTICATED USERS
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 6. STRICT OWNER-ONLY POLICIES FOR AUTHENTICATED USERS
-- (Non-admins receive 0 rows; anonymous users receive 0 rows)
-- ------------------------------------------------------------------------------

CREATE POLICY "admin_all_borrowers" ON public.borrowers
  FOR ALL TO authenticated
  USING (public.is_finexa_admin())
  WITH CHECK (public.is_finexa_admin());
...
CREATE POLICY "admin_all_penalty_ledger" ON public.penalty_ledger
  FOR ALL TO authenticated
  USING (public.is_finexa_admin())
  WITH CHECK (public.is_finexa_admin());
...
CREATE POLICY "admin_all_loan_applications" ON public.loan_applications
  FOR ALL TO authenticated
  USING (public.is_finexa_admin())
  WITH CHECK (public.is_finexa_admin());

CREATE POLICY "admin_all_funders" ON public.funders
  FOR ALL TO authenticated
  USING (public.is_finexa_admin())
  WITH CHECK (public.is_finexa_admin());

CREATE POLICY "admin_all_capital_returns" ON public.capital_returns
  FOR ALL TO authenticated
  USING (public.is_finexa_admin())
  WITH CHECK (public.is_finexa_admin());

CREATE POLICY "admin_all_loan_cycles" ON public.loan_cycles
  FOR ALL TO authenticated
  USING (public.is_finexa_admin())
  WITH CHECK (public.is_finexa_admin());

CREATE POLICY "admin_all_payment_reminders" ON public.payment_reminders
  FOR ALL TO authenticated
  USING (public.is_finexa_admin())
  WITH CHECK (public.is_finexa_admin());

CREATE POLICY "admin_all_admin_notifications" ON public.admin_notifications
  FOR ALL TO authenticated
  USING (public.is_finexa_admin())
  WITH CHECK (public.is_finexa_admin());

CREATE POLICY "admin_all_completed_notification_keys" ON public.completed_notification_keys
  FOR ALL TO authenticated
  USING (public.is_finexa_admin())
  WITH CHECK (public.is_finexa_admin());

CREATE POLICY "admin_all_push_subscriptions" ON public.push_subscriptions
  FOR ALL TO authenticated
  USING (public.is_finexa_admin())
  WITH CHECK (public.is_finexa_admin());

CREATE POLICY "admin_all_pushed_notification_keys" ON public.pushed_notification_keys
  FOR ALL TO authenticated
  USING (public.is_finexa_admin())
  WITH CHECK (public.is_finexa_admin());
...
CREATE POLICY "admin_all_notifications_log" ON public.notifications_log
  FOR ALL TO authenticated
  USING (public.is_finexa_admin())
  WITH CHECK (public.is_finexa_admin());
