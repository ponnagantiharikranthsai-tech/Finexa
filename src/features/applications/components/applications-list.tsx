...
export function ApplicationsList({ initialApps, total: initialTotal, totalPages: initialTotalPages }: ApplicationsListProps) {
  ...
  // Generate WhatsApp bot message from application data
  const getAppBotMessage = (app: ApplicationWithBorrower) => {
    const principalNum = Number(app.principal || 0);
    const interestAmtNum = Number(app.interestAmount || 0);
    const calculatedRate = principalNum > 0
      ? ((interestAmtNum * 1000) / principalNum).toFixed(0)
      : "20";

    return formatLoanApprovalMessage({
      fullName: app.borrower?.name || app.customerName || "Borrower",
      phoneNumber: app.borrower?.mobile || app.customerMobile || "N/A",
      principal: app.principal,
      interestRate: calculatedRate,
      monthlyInterest: app.interestAmount,
      termStartDate: app.startDate,
      termEndDate: app.dueDate,
    });
  };

  const getWhatsAppRedirectUrl = (app: ApplicationWithBorrower) => {
    const rawMobile = app.borrower?.mobile || app.customerMobile || "";
    let cleanMobile = rawMobile.replace(/[^0-9]/g, "");
    if (cleanMobile.length === 10) {
      cleanMobile = `91${cleanMobile}`;
    }
    const msg = getAppBotMessage(app);
    return cleanMobile
      ? `https://wa.me/${cleanMobile}?text=${encodeURIComponent(msg)}`
      : `https://api.whatsapp.com/send?text=${encodeURIComponent(msg)}`;
  };

  const handleOpenWhatsApp = (app: ApplicationWithBorrower) => {
    const url = getWhatsAppRedirectUrl(app);
    window.open(url, "_blank");
  };

  const handleCopyBotMessage = (app: ApplicationWithBorrower) => {
    const msg = getAppBotMessage(app);
    copyToClipboard(msg);
        ...
        <DialogContent className="max-w-[750px] w-[95vw] max-h-[85vh] overflow-y-auto rounded-2xl border border-border p-0 bg-white dark:bg-card">
          {selectedApp && (
            <div>
              ...
              <div className="p-6 space-y-6">
                ...
                {selectedApp.borrower || selectedApp.customerName ? (
                  <>
                    ...
                    {/* WhatsApp Bot Confirmation Message Card */}
                    <div className="p-4 rounded-xl border border-emerald-500/20 bg-emerald-500/5 dark:bg-emerald-950/15 space-y-3">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                        <div className="flex items-center gap-2">
                          <svg className="h-4 w-4 text-emerald-600 dark:text-emerald-400 shrink-0" fill="currentColor" viewBox="0 0 24 24">
                            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L0 24l6.335-1.662c1.746.953 3.71 1.458 5.704 1.459h.008c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                          </svg>
                          <span className="text-xs font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-400">
                            WhatsApp Bot Approval Message
                          </span>
                        </div>
                        <div className="flex items-center gap-2">
                          <Button
                            ...
                            variant="outline"
                            size="sm"
                            onClick={() => handleCopyBotMessage(selectedApp)}
                            className="h-8 text-xs font-semibold rounded-lg border-border hover:bg-secondary flex items-center gap-1"
                          >
                            <Copy className="h-3.5 w-3.5" />
                            Copy Message
                          </Button>
                          <Button
                            type="button"
                            size="sm"
                            onClick={() => handleOpenWhatsApp(selectedApp)}
                            className="h-8 text-xs font-semibold rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white flex items-center gap-1.5 shadow-sm"
                          >
                      ...
                      <pre className="text-xs font-mono p-3.5 rounded-xl bg-background/90 border border-border/70 whitespace-pre-wrap text-foreground max-h-48 overflow-y-auto leading-relaxed select-all">
                        {getAppBotMessage(selectedApp)}
                      </pre>
                    </div>
                  </>
                ) : (
              ...
              {/* Action Footer */}
              <div className="px-6 py-4 border-t border-border flex flex-wrap items-center justify-between gap-3 bg-secondary/20">
                <div className="flex items-center gap-2">
                  {(selectedApp.borrower || selectedApp.customerName) && (
                    <>
                      <Button
                        ...
                        variant="outline"
                        onClick={() => handleCopyBotMessage(selectedApp)}
                        className="rounded-xl text-xs font-bold h-9 flex items-center gap-1.5 border-border hover:bg-secondary"
                      >
                        <Copy className="h-3.5 w-3.5" />
                        Copy Bot Message
                      </Button>
                      <Button
                        type="button"
                        onClick={() => handleOpenWhatsApp(selectedApp)}
                        className="rounded-xl text-xs font-bold h-9 bg-emerald-600 hover:bg-emerald-700 text-white flex items-center gap-1.5 shadow-sm"
                      >
                        <svg className="h-3.5 w-3.5" fill="currentColor" viewBox="0 0 24 24">
                          <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L0 24l6.335-1.662c1.746.953 3.71 1.458 5.704 1.459h.008c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                          </svg>
                    ...
                    {selectedApp.status === "pending_verification" && (
                      <>
                        <Button
                          ...
                          disabled={isPending}
                          variant="destructive"
                          className="rounded-xl font-semibold text-xs px-4 h-9 flex items-center gap-1.5 shadow-md active:scale-[0.98] transition-all disabled:opacity-60"
                        >
                          Reject
                        </Button>
                        <Button
                          ...
                          disabled={isPending}
                          className="rounded-xl font-semibold text-xs px-4 h-9 bg-[#FFD54A] hover:bg-[#FFE082] text-black flex items-center gap-1.5 shadow-md active:scale-[0.98] transition-all disabled:opacity-60"
                        >
